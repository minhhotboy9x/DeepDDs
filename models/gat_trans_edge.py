...
class EdgedGATTransNet(torch.nn.Module):
    def __init__(self, num_features_xd=78, n_output=2, num_features_xt=954, output_dim=128, dropout=0.2, file=None):
        ...
        # Transformer
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=output_dim, nhead=8, dim_feedforward=512)
    ...
    def tranformer_forward(self, x, batch):
        attn_mask = batch.unsqueeze(0) != batch.unsqueeze(1)  # Shape (num_node, num_node)
    ...
    def forward(self, data1, data2):
        x1, edge_index1, edge_attr1, batch1, cell = data1.x, data1.edge_index, \
            data1.edge_attr, data1.batch, data1.cell
        
        x2, edge_index2, edge_attr2, batch2 = data2.x, data2.edge_index, \
            data2.edge_attr, data2.batch
        # deal drug1
        # begin_x1 = np.array(x1.cpu().detach().numpy())
        x1 = self.drug1_gcn1(x1, edge_index1, edge_attr1)
        x1 = F.elu(x1)
        x1 = F.dropout(x1, p=0.2, training=self.training)
        x1 = self.drug1_gcn2(x1, edge_index1, edge_attr1)
        x1 = F.elu(x1)
        x1 = F.dropout(x1, p=0.2, training=self.training)
        x1 = self.tranformer_forward(x1, batch1)
        ...
        x2 = self.drug1_gcn1(x2, edge_index2, edge_attr2)
        x2 = F.elu(x2)
        x2 = F.dropout(x2, p=0.2, training=self.training)
        x2 = self.drug1_gcn2(x2, edge_index2, edge_attr2)
        x2 = F.elu(x2)
        x2 = F.dropout(x2, p=0.2, training=self.training)
        x2 = self.tranformer_forward(x2, batch2)
        # fin_x2 = np.array(x2.cpu().detach().numpy())


        # if len(batch1) < 1000:
        #     dt = pd.DataFrame(begin_x2)
        #     # dt.to_csv('data/case_study/begin_fin/' + self.filename + '_drug1_begin_x2.csv', header=None, index=None)
        #     # dt = pd.DataFrame(fin_x2)
        #     # dt.to_csv('data/case_study/begin_fin/' + self.filename + '_drug1_fin_x2.csv', header=None, index=None)
        #
